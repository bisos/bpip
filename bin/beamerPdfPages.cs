...
** This File: /bisos/git/auth/bxRepos/bisos/bpip1/bin/beamerPdfPages.cs
...
import typing
csInfo: typing.Dict[str, typing.Any] = { 'moduleName': ['beamerPdfPages'], }
csInfo['version'] = '202211035047'
csInfo['status']  = 'inUse'
csInfo['panel'] = 'beamerPdfPages-Panel.org'
...
####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "updatePages" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 1 :argsMax 1 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<updatePages>>  =verify= argsMin=1 argsMax=1 ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class updatePages(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  Given the pdf output of beamer as =inFile= update slide pdf pages in ./disposition.gened
        Walk through  disposition.gened/slideNu, determine frameName.
        create a slide.pdf in coresponding frameName.
        #+end_org """)
        ...
        dispositionGenedPath = pathlib.Path(inFile).parent.joinpath('disposition.gened')

        if not dispositionGenedPath.exists():
            return(b_io.eh.badOutcome(cmndOutcome))

        for i in itertools.count():
            nu = i+1
            slideNuPath = dispositionGenedPath.joinpath(f"slide{nu}")
            if not slideNuPath.is_dir():
                break
            slideFrameNamePar = b.fp.FileParamReadFromPath(slideNuPath)
